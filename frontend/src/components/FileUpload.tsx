...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const processMarkdown = (markdown: string): string => {
    // Sanitize but keep <br> tags
    let sanitized = DOMPurify.sanitize(markdown, {
      ALLOWED_TAGS: ['p', 'strong', 'em', 'ul', 'li', 'table', 'thead', 'tbody', 'tr', 'th', 'td', 'br'],
      ALLOWED_ATTR: [],
    });

    // Replace <br> tags with real newlines to separate points inside cells
    sanitized = sanitized.replace(/<br\s*\/?>/gi, ' \n');

    // Convert bullet symbol • to markdown list item -
    sanitized = sanitized.replace(/•\s?([^\n]+)/g, '- $1');

    // Fix coverage sections formatting if necessary
    sanitized = sanitized.replace(/\*\*WHAT'S COVERED:\*\*\n/g, '**WHAT\'S COVERED:**\n');
    sanitized = sanitized.replace(/\*\*WHAT'S NOT COVERED:\*\*\n/g, '**WHAT\'S NOT COVERED:**\n');

...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const responseTypeConfig: Record<string, { bgColor: string; borderColor: string }> = {
    ...
    policy_summary: { bgColor: 'bg-blue-50', borderColor: 'border-blue-200' },
    similar_search: { bgColor: 'bg-teal-50', borderColor: 'border-teal-200' },
    specific_person: { bgColor: 'bg-green-50', borderColor: 'border-green-200' },
    comparison: { bgColor: 'bg-purple-50', borderColor: 'border-purple-200' },
    coverage_check: { bgColor: 'bg-indigo-50', borderColor: 'border-indigo-200' },
    limits_deductibles: { bgColor: 'bg-yellow-50', borderColor: 'border-yellow-200' },
    open_ended: { bgColor: 'bg-orange-50', borderColor: 'border-orange-200' },
    general_inquiry: { bgColor: 'bg-gray-50', borderColor: 'border-gray-200' },
    no_results: { bgColor: 'bg-red-50', borderColor: 'border-red-200' },
  ...
  const processMarkdown = (markdown: string): string => {
    ...
    // FIX #2: Merge stray text (like in the Lemonade example) back into a table row format
    sanitizedMarkdown = sanitizedMarkdown.replace(/•\s*([^|\n]+)\s*\|\s*•\s*([^|\n]+)/g, (match, inclusion, exclusion) => {
      return `| | • ${inclusion.trim()}<br> | • ${exclusion.trim()} |`;
    });

    // FIX #1: Replace <br> tags with a Markdown hard break (two spaces + newline)
    sanitizedMarkdown = sanitizedMarkdown.replace(/<br\s*\/?>/gi, '  \n');

    // Process any remaining bullet points (•) to convert to standard Markdown list syntax
    sanitizedMarkdown = sanitizedMarkdown.replace(/•\s*([^\n]+)/g, '- $1');
    ...
    // If references are provided separately, append them
